...
export async function getCompanyProfile(symbol: string): Promise<import('../types').CompanyProfile | null> {
    const data = await fetchWithRetry<any>(`${BASE_URL}/stock/profile2?symbol=${symbol}&token=${API_KEY}`);
...
export async function getCompanyMetrics(symbol: string): Promise<import('../types').CompanyMetrics | null> {
    const data = await fetchWithRetry<any>(`${BASE_URL}/stock/metric?symbol=${symbol}&metric=all&token=${API_KEY}`);
    if (data && data.metric) {
...
export async function searchSymbol(query: string) {
    const data = await fetchWithRetry<{ result: any[] }>(`${BASE_URL}/search?q=${encodeURIComponent(query)}&token=${API_KEY}`);
...
export async function getQuote(symbol: string): Promise<Quote | null> {
    const data = await fetchWithRetry<any>(`${BASE_URL}/quote?symbol=${symbol}&token=${API_KEY}`);
...
export async function getCompanyNews(symbol: string): Promise<NewsArticle[]> {
    ...
    const data = await fetchWithRetry<NewsArticle[]>(`${BASE_URL}/company-news?symbol=${symbol}&from=${from}&to=${to}&token=${API_KEY}`);
...
export async function getHistory(symbol: string, range: string): Promise<HistoryData | null> {
    ...
    const data = await fetchWithRetry<any>(`${BASE_URL}/stock/candle?symbol=${symbol}&resolution=${resolution}&from=${Math.floor(from)}&to=${to}&token=${API_KEY}`);
